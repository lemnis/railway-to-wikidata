...
export const getLocations = async () => {
  ...
  return merged.map<LocationV4>((location) => ({
    labels: [{ value: location.name }],
    claims: {
      [Property.CoordinateLocation]: [
        { value: [parseFloat(location.loc.lat), parseFloat(location.loc.lng)] },
      ],
      [Property.StationCode]: [{ value: location.stationList?.value }],
    },
  }));
};
